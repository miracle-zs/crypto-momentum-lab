...
class PostgresPositionReservationRepository:
    """Async PostgreSQL repository for durable lot reservations."""
        ...
        async with self._session_maker() as session, session.begin():
            stmt = (
                ...
                .values(
                    ...
                    strategy_name=reservation.position_key.strategy_name,
    ...
    async def load_active_reservations(
        ...
        """Load all active reservations from PostgreSQL."""
        async with self._session_maker() as session:
            ...
            if key is not None:
                query = query.where(
                    ...
                    PositionReservationRow.strategy_name == key.strategy_name,
    ...
    async def release_expired_reservations(self, before: datetime) -> int:
        """Release active reservations whose expires_at is older than before."""
        now = datetime.now(UTC)
        async with self._session_maker() as session, session.begin():
            stmt = (
                update(PositionReservationRow)
