...
function equityChart(rows) {
  ...
  return `<div class="equity-chart"><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Paper account equity curve"><defs><linearGradient id="equity-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#4cc9c0" stop-opacity=".34"/><stop offset="1" stop-color="#4cc9c0" stop-opacity="0"/></linearGradient></defs><line x1="${pad}" y1="${pad}" x2="${width - pad}" y2="${pad}" class="chart-grid"/><line x1="${pad}" y1="${height - pad}" x2="${width - pad}" y2="${height - pad}" class="chart-grid"/><polygon points="${area}" fill="url(#equity-fill)"/><polyline points="${line}" class="equity-line"/><text x="${pad}" y="18">${esc(money(max))}</text><text x="${pad}" y="${height - 8}">${esc(money(min))}</text><text x="${pad}" y="${height - 8}" text-anchor="start">${esc(first)}</text><text x="${width - pad}" y="${height - 8}" text-anchor="end">${esc(last)} UTC</text></svg></div>`;
}
