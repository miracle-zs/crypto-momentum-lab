...
6. **本地维护收敛（本轮追加）**：#35 将 state/quote hub 的字符串和 datetime 字段校验收敛到共享解析 helper，保留各自异常类型与 decimal 输入契约；#36 将 PostgreSQL JSON 序列化收敛到共享 helper；#37 将 paper/daemon 候选成交边界收敛到共享解析函数；#38 将 paper/replay 报告序列化收敛到共享 helper；#41 让配置了 coordinator 的 capture service 通过 coordinator 提交，保留磁盘保护与队列溢出状态机；#42 将 active entry symbol 的 EXTENDED 过滤下推到 SQL。strategy runner、PostgreSQL persistence 与 market-data 相关回归共 301 项通过（market-data 集合内包含 hub 测试，另有 2 项需本机回环权限的既有跳过项）。鉴权 D8 按当前要求暂缓。
