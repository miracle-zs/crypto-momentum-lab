...
- P2 #28（第一阶段）：陈旧 paper market state 现在按 symbol 只告警一次，并在恢复新鲜 state 时记录恢复事件；陈旧期间明确跳过策略和持仓标记，退出延后到新鲜行情，避免用陈旧价格模拟成交。无行情输入本身的独立 watchdog 和恢复后的即时退出规则仍待补齐。
- D1–D3、D5、D9：按需选择 live compose、修正 strategies 路径、多账户 ops monitor、live position label 预检、补齐 gainer10 的部署断言。

验收方式：相关 unit/smoke/e2e 测试、strategy runner 与 PostgreSQL persistence unit（146 passed）、PostgreSQL `tests/integration`（50 passed）、ruff、compileall 和 `git diff --check` 已通过；迁移 `20260910_0032` 已在同一 PostgreSQL 容器上用本地 `.venv` 的 Alembic 升级到 head。Docker Compose 的 `migrate` 一次性任务仍受 Docker Hub BuildKit frontend 拉取 EOF 影响；一个既有的 market-data cleanup 测试会挂起，已单独隔离。未连接真实服务器，也未执行容器重启。
...
3. **P2 正确性收敛**：继续处理 24–34，优先 #28 的无行情 watchdog/恢复退出规则、#32 的 fill 重连结果语义与剩余边界；#24 的本地缺口可观测性、#25 的游标化官方 candle 顺序回补、#28 的陈旧状态告警、#26/#27/#29/#33/#34 已在第一批收敛。
