...
4. **架构收敛**：A6 已完成第一阶段：运行时 checkpoint 现在保留 1m 闭合 K 线元数据、数据完整性和缺失 aggTrade 计数，并通过完整字段与旧 checkpoint 默认值的 round-trip 回归测试；A2、A4、A7 在补齐调用方测试后再重构，暂不做大规模 compose 或入口重写。
