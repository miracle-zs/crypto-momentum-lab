...
5. **部署条件项**：回滚分支、D1/D2/D3/D4/D5/D6/D7/D9 已完成第一阶段；D6 统一四个 live 账户的 `submit,cancel` 默认，并保留显式 `all` 诊断开关。部署回归通过。仍需确认真实 nginx 上层鉴权和数据库备份/恢复演练证据；若证据不足，再分别补 health probe 兼容的应用鉴权和可演练的备份作业。
