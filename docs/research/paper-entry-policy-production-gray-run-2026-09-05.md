...
观察时间约为 18 分钟（11:17:15–11:35:15 UTC），覆盖一个 15 分钟策略周期。使用
`--max-mismatches 0 --max-mismatch-rate 0 --fail-on-alert` 运行严格报告：
...
阶段 B 当前通过一个完整周期的初步门槛，但仍需继续积累更长基线并由人工确认是否扩大
Paper 覆盖。任何健康失败、观测写入阻塞、无法解释的差异或数据库/市场数据异常，立即移除
