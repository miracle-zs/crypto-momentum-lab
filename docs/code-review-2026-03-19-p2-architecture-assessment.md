...
复核日期：2026-09-10。对象：docs/code-review-2026-03-19.md。依据为当前工作区代码及配置，不代表报告日期对应的历史版本或线上部署状态。本轮更新 #24–#43、A2、A4、A6、A7、A11 的验收记录并包含对应的本地修复或不修结论。
...
| 编号 | 结论 | 证据、适用范围与处理意见 |
|---|---|---|
| #24 | 已完成第一阶段，官方源为权威边界 | [Candle15mAggregator](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/portfolio.py:85) 对不完整、缺分钟和乱序窗口记录有界 gap，并只发出完整 K 线；配置 [BinanceRestClosedCandle15mSource](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/candle_source.py:184) 时，daemon 不使用本地合成 candle。官方源的 HTTP、超时、重试耗尽和 malformed response 统一为 `ClosedCandleSourceError`，daemon 保留仓位、告警并按 30 秒退避重试，不用残缺数据平仓。 |
| #25 | 已完成第一阶段，旧持仓采用保守初始化 | [paper position](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/persistence/postgres/paper_daemon_repository.py:233) 持久化 `last_candle_end`；[回补函数](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/daemon.py:1203) 按游标顺序处理官方闭合 candle，覆盖 `confirmation_count>1` 与 grace。迁移前游标为 NULL 的持仓不从 `opened_at` 猜测历史，只检查当前最近完整 15m 窗口；首次成功处理后写入游标，并发出一次 `paper_legacy_candle_cursor_fallback` 告警。 |
...
| #30 | 已处理，未接线的操作入口已删除 | 全 src 未找到带 `RollbackCommand` 参数的 `BinanceUsdMTradeClient.cancel_order` 调用；当前 live 的开仓挂单撤销与退出订单取消统一走 coordinator/state machine 或 `cancel_order_by_client_id`，平仓仍保留 reduce-only 执行链路。已删除这个没有运营调用方、错误契约也未统一的手动撤单入口及其专门测试；保留内部订单生命周期取消和独立的紧急 reduce-only 平仓授权入口。 |
...
| 编号 | 结论 | 证据与处理意见 |
|---|---|---|
| #35 | 已完成第一阶段 | [state/quote hub](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/market_data/protocol_parsing.py:4)现在共用字符串和 timezone-aware datetime 解析 helper；quote 仍只接受 decimal string，state 仍保留更宽的数值输入与 sequence/replay 协议，两个 hub 继续抛出各自异常类型。两个 hub 的回归测试 17 项通过，1 项因本机回环权限既有跳过。 |
| #36 | 已完成 | [order repository](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/persistence/postgres/order_repository.py:36) 与 [account repository](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/persistence/postgres/account_repository.py:29) 共用 [jsonable](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/persistence/postgres/serialization.py:8)，保留枚举、Decimal、时区 datetime、容器及 fallback 字符串语义。序列化和 account repository 回归 3 项通过。 |
| #37 | 已完成 | [paper](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/paper.py:33) 与 [daemon](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/daemon.py:38) 共用 `resolve_candidate_fill_at_state`，统一目标时间、过期时间和闭合状态边界；paper/daemon/fills 回归 48 项通过。 |
| #38 | 已完成 | [paper](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/paper.py:52) 与 [replay](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/replay.py:39) 共用 [strategy_runner.serialization.jsonable](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/strategy_runner/serialization.py:8)，统一 dataclass、枚举、Decimal、datetime、容器和 fallback 字符串转换。paper/replay/report 序列化回归 28 项通过。 |
| #39 | 已核验，不作为当前缺陷 | state machine [默认开启全实例锁](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/execution_account/orders/state_machine.py:186)属实，但两个 live 构建点均显式 `serialize_commands=False`：[一次性执行](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/apps/live_rollout/main.py:1403)、[daemon](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/apps/live_rollout/main.py:1982)。shadow 不经同样的 coordinator 包装时保留默认锁有合理性；未来调用者误配属于 API 易用性风险，不是当前 live 性能 bug。state machine/live CLI 回归 73 项通过。 |
| #40 | 已核验，不作为缺陷 | [execute_approved_intent](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/execution_account/orders/coordinator.py:195)只是转发给 `submit`，注释明确为兼容旧调用名，没有第二份算法。现有 coordinator/state-machine 回归 19 项通过；在所有调用方迁移前删除会破坏兼容性。 |
| #41 | 已完成 | [service.submit](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/market_data/capture/service.py:177) 在配置 coordinator 时直接委托其 `submit`，同时保留 service 的磁盘检查、队列溢出转 HALT 和无 coordinator 的兼容路径；生产 websocket 入口继续使用 [coordinator.submit](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/apps/market_data/main.py:802)。capture service/coordinator/market-data CLI 回归 41 项通过。 |
| #42 | 已完成 | [load_active_entry_symbols_at](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/persistence/postgres/repository.py:245) 现在直接在数据库端选择目标快照中非 `EXTENDED` 的 symbol，保留最新 activated snapshot 与 `observed_at` 截止语义。PostgreSQL 集成回归 5 项通过。 |
| #43 | 已核验，不作为冗余缺陷 | [双 watermark 逻辑](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/market_data/runtime_states.py:560)允许 realtime 发布后继续接收迟到数据，再构建 durable 状态；到 durable 关闭时才[删除 accumulator 和 bucket quote](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/market_data/runtime_states.py:662)。bucket 最新 quote、realtime 前值、durable 前值服务不同时间边界，合并会改变报价语义。runtime state/quote hub 回归 15 项通过，保留现有生命周期设计。 |
...
本轮没有处理鉴权 D8，也不新增备份 D10；两项按当前要求保留为外部证据/运营语义待定。A8 的 `save_command` 仅保留待运营语义确认，不作为删除项。
