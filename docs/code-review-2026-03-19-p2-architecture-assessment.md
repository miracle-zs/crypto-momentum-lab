...
| 编号 | 结论 | 证据与处理意见 |
|---|---|---|
| A1 | 重复成立，但 URL 差异是合理的 | [SQLAlchemy 探针](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/apps/healthcheck.py:1)与[psycopg 探针](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/apps/healthcheck_fast.py:1)重复维护 readiness 判定；当前两个 compose 的应用探针使用 cml-local-healthcheck。应表述为“当前仓库部署配置未调用”，不能据此确认线上无人使用。_sync_database_url 不一致并非错误：SQLAlchemy 需要 postgresql+psycopg，psycopg 直接连接需要 postgresql。fast 版还显式设置 connect/statement timeout，不能说只差 SQL 占位符。保留一个手动 DB 诊断入口合理，research_collector 的独立健康检查也不因文件多而自动冗余。 |
...
| 项 | 当前调用证据 | 验收 |
|---|---|---|
| load_run_summary | tests/integration/persistence/test_strategy_run_repository.py:54；test_paper_daemon_artifacts.py:163 | 无已发现 src 生产调用，有真实测试调用；“仅定义”错误。 |
| load_paper_report_artifacts | tests/integration/persistence/test_strategy_run_repository.py:80 | 同上。 |
| count_quality_events | tests/integration/persistence/test_capture_repository.py:53 | 测试读取接口，可讨论是否移至测试 helper。 |
| latest_process_state | tests/integration/persistence/test_capture_repository.py:55；tests/e2e/test_market_data_process.py:78 | 不止定义，E2E 会使用。 |
| save_command | src/crypto_momentum_lab/persistence/postgres/live_rollout_repository.py:104；全 src/tests/scripts 搜索未见调用 | 当前无调用成立；需确认 rollback command 的审计契约再决定接线或删除。 |
...
- 运行 portfolio、order coordinator、healthcheck_fast 三组既有单测：25 passed。它们不是全部验收项的端到端测试，也不覆盖新复现的组合缺口。
