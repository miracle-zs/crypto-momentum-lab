...
| D6：遥测持久化默认值不同 | 事实成立，非交易账本错误 | primary [默认空](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/compose.server.yaml:594)经 [_parse_exchange_operations](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/apps/live_rollout/main.py:4082)变成 None；附加账户为 submit,cancel。[telemetry](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/live_rollout/telemetry.py:393)此 allow-list 筛选的是 exchange 请求/响应遥测，不是 order/fill 账本。查询类排障数据覆盖不同、写库负载不同成立，“对不上账”不准确。可统一默认或记录明确的观测策略，不应为整齐一律恢复全量写入。 |
| D7：env 与 argv 重复 | 重复存在，但“env 虚设”和删除建议不成立 | 四个账户的七维参数在 env/argv 中使用完全相同的 Compose 插值表达式，已逐项核对；修改 .env 输入会同时改变两者，不需要手动改两份。更重要：[preflight hash resolver](/Users/zhangshuai/PycharmProjects/crypto-momentum-lab/src/crypto_momentum_lab/apps/live_rollout/main.py:4279)读取 env；部署脚本 run ... preflight/refresh-approval-runtime 会替换 service 默认 command，无法继承 daemon argv。删掉 environment 会使一次性校验/审批回落 primary 默认参数，尤其破坏 account-3/4 的配置 hash。正确方向是统一配置解析/生成，并确保 daemon 与一次性命令获得相同参数；可考虑删冗余 argv 改为共同 env，但必须连同入口测试验证。 |
